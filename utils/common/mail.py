...
logger = Logger(logname=filename).get_logger()
class Email:
    def __init__(self, server, sender, password, receiver, title, message=None, path=None,htmlReportList=None):
        """初始化Email
        ...
        :param htmlReportList:测试报告列表，非必填,用于获取最新报告的参数。
        """
        self.title = title
        ...
        self.htmlReportList = htmlReportList

    def new_report(self):
        htmlfiles=os.listdir(self.htmlReportList)
        sortHtmlFileList = sorted(htmlfiles, key=lambda x: os.path.getmtime(htmlfiles))
        latelyHtmlFile = sortHtmlFileList[-1]
        return latelyHtmlFile
    ...
    def send(self):
        ...
        if file_new:
            self.msg.attach(MIMEText(open(file_new,'rb').read(),'html','utf-8'))
